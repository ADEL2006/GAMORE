  ...
  return (
    <div>
      <ul>
        {boardList.map((board) => (
          // 4. map 함수로 데이터 출력
          <li key={board.idx} class="board-list">
            <Link to={`/board/${board.idx}`}>{board.title}</Link>
          </li>
        ))}
      </ul>
      <div>
        <button onClick={onClick} value={1} class="page-block">
          &lt;&lt;
        </button>
        <button onClick={onClick} value={prevBlock} class="page-block">
          &lt;
        </button>
        {pageList.map((page, index) => (
          <button key={index} onClick={onClick} value={page} class="page-block">
            {page}
          </button>
        ))}
        <button onClick={onClick} value={nextBlock} class="page-block">
          &gt;
        </button>
        <button onClick={onClick} value={lastPage} class="page-block">
          &gt;&gt;
        </button>
      </div>
      <br />
      <div>
        <select name="sk" onChange={onChange} class="search-form">
          <option value="">-선택-</option>
          <option value="title">제목</option>
          <option value="contents">내용</option>
        </select>
        <input type="text" name="sv" id="" onChange={onChange}/>
        <button onClick={onSearch} class="search">검색</button>
      </div>
      <br />
      <div>
        <button onClick={moveToWrite} class="write">글쓰기</button>
      </div>
    </div>
  );
};
